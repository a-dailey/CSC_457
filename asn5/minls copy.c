...
int list_directory(int fd, struct superblock *sb, uint32_t partition_offset, char *path) {
    ...
    //get correct inode offset
    off_t inode_table_offset = partition_offset + (sb->i_blocks * sb->blocksize) + (sb->z_blocks * sb->blocksize);
    off_t root_inode_offset = inode_table_offset + (1 - 1) * sizeof(struct inode); // Root inode (inode 1)



    //Seek to the inode within the selected partition
    if (lseek(fd, inode_table_offset, SEEK_SET) < 0) {
    ...
    for (i = 0; i < DIRECT_ZONES; i++) {
        ...
        uint32_t zone_offset = partition_offset + ((sb->firstdata + inode.zone[i]) * sb->blocksize);

        
        uint32_t j;
        for (j = 0; j < sb->blocksize / sizeof(struct dir_entry); j++) {
